...
impl LogEntry {
    pub fn from_adif_record(record: &crate::adif::AdifRecord) -> Result<LogEntry, ImportError> {
        match (
            Call::call_from_adif_record(record),
            Call::my_call_from_adif_record(record),
            qso_freq(record),
            qso_date(record),
            record.adir_field_values.get("mode"),
            record.adir_field_values.get("comment"),
            record.adir_field_values.get("lotw_qsl_sent"),
            record.adir_field_values.get("lotw_qsl_rcvd"),
        ) {
            (
                Some(call),
                Some(my_call),
                ...
                let lotw_sent = match lotw_sent {
                    Some(x) if x == "Y" => true,
                    _ => false,
                };
                let lotw_rcvd = match lotw_rcvd {
                    Some(x) if x == "Y" => true,
                    _ => false,
                };
...
fn qso_date(record: &crate::adif::AdifRecord) -> Option<DateTime<Utc>> {
    match (
        record.adir_field_values.get("qso_date"),
        record.adir_field_values.get("time_on"),
    ) {
